...
class DPoserComp(object):
    ...
    @torch.no_grad()
    def one_step_denoise(self, x_t, t, condition=None, mask=None):
        drift, diffusion, alpha, sigma_2, score = self.rsde.sde(x_t, t, guide=True, condition=condition, mask=mask)
        x_0_hat = (x_t + sigma_2[:, None] * score) / alpha

        return x_0_hat.detach()

    @torch.no_grad()
    def multi_step_denoise(self, x_t, t, t_end, N=10, condition=None, mask=None):
        ...
        for i in range(N):
            ...
            score = self.score_fn(x_current, t_current, condition=condition, mask=mask)
    ...
    def loss(self, x_0, vec_t, condition=None, mask=None, weighted=True, multi_denoise=False):
        # x_0: [B, j*6], vec_t: [B], condition: [B, text_dim], mask: [B, j*6]
        ...
        if multi_denoise:  # not recommended
            ...
            denoise_data = self.multi_step_denoise(perturbed_data, vec_t, t_end=t_end, N=10, condition=condition, mask=mask)
        else:
            denoise_data = self.one_step_denoise(perturbed_data, vec_t, condition=condition, mask=mask)
    ...
    def improved_loss(self, x_0, x_ref, vec_t, condition=None, mask=None, weighted=True, multi_denoise=False):
        # x_0: [B, j*6], x_ref, [B, j*6] vec_t: [B], condition: [B, text_dim], mask: [B, j*6]
        z = torch.randn_like(x_0)
        mean, std = self.sde.marginal_prob(x_0, vec_t)
        ...
        if multi_denoise:  # not recommended
            t_end = torch.ones_like(vec_t) * 0.001
            denoise_x_0 = self.multi_step_denoise(noisy_x_0, vec_t, t_end=t_end, N=5, condition=condition, mask=mask)
            denoise_x_ref = self.multi_step_denoise(noisy_x_ref, vec_t, t_end=t_end, N=5, condition=condition, mask=mask)
        else:
            denoise_x_0 = self.one_step_denoise(noisy_x_0, vec_t, condition=condition, mask=mask)
            denoise_x_ref = self.one_step_denoise(noisy_x_ref, vec_t, condition=condition, mask=mask)
    ...
    def optimize(self, observation, mask, condition=None, time_strategy='3', lr=0.1,
            ...
            for it in range(iterations):
                for i in range(steps_per_iter):
                    ...
                    if self.improve_baseline:
                        loss_dict['dposer'] = self.improved_loss(opti_variable, baseline_data, vec_t, condition=condition, mask=mask)
                    else:
                        loss_dict['dposer'] = self.loss(opti_variable, vec_t, condition=condition, mask=mask)
