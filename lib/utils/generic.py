...
def load_model(model, config, ckpt_path, device, is_ema=True):
    """Fixed load_model for PyTorch 2.6+ (resolves UnpicklingError with numpy scalars)"""
    # Step 1: Whitelist numpy scalars (required for PyTorch 2.6+ weights_only=True)
    add_safe_globals([np.core.multiarray.scalar])
    
    ema = ExponentialMovingAverage(model.parameters(), decay=config.ema_rate)
    
    # Step 2: Load checkpoint with weights_only=False (safe for trusted checkpoints)
    if ckpt_path.endswith('.ckpt'):
