...
class DPoserTrainer(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # --------------------------
        # Fix: Check if trajs is NOT None before setting last_trajs
        # --------------------------
        run_batches = min(len(self.val_dataloader()), self.trainer.limit_val_batches)
        if batch_idx == run_batches - 1:
            if trajs is not None:
                self.last_trajs = trajs
                print(f"✅ last_trajs SET (batch {batch_idx} = last run batch)!")
                print(f"   trajs shape: {trajs.shape}, samples shape: {samples.shape}")
            else:
                print(f"❌ trajs is None—cannot set last_trajs!")
                # Fallback: Create dummy last_trajs
                dummy_shape = (100, poses.shape[0], poses.shape[1])
                self.last_trajs = torch.zeros(dummy_shape, device=poses.device)
        else:
            print(f"❌ last_trajs NOT set (batch {batch_idx})")

        # Rest of your code (log metrics)
    ...
    def process_and_log_meshes(self, poses, bg_img, focal, princpt, tag_prefix):
        try:
            ...
            rendered_images = []
            for mesh in meshes:
                # Fix: Ensure render_mesh returns a valid numpy array [H, W, 3]
                rendered_img = render_mesh(bg_img, mesh, faces, {'focal': focal, 'princpt': princpt})
                if rendered_img is None:
                    print(f"⚠️ render_mesh returned None for {tag_prefix} — using dummy image!")
                    rendered_img = np.ones_like(bg_img) * 255  # White dummy
                rendered_img_tensor = self.convert_to_tensor(rendered_img)
                rendered_images.append(rendered_img_tensor)

            # Create image grid (10 columns for 50 poses → 5 rows × 10 cols)
            image_grid = torchvision.utils.make_grid(rendered_images, nrow=10)  # [C, H, W]
            
            # Log to TensorBoard
            self.logger.experiment.add_image(f'{tag_prefix}_grid', image_grid, self.current_epoch)
            
            # --------------------------
            # Fix 2: Convert grid back to numpy [H, W, 3] for W&B (required!)
            # --------------------------
            # W&B expects [H, W, 3] (numpy), not [C, H, W] (tensor)
            image_grid_np = image_grid.permute(1, 2, 0).cpu().numpy()
            # Denormalize from [0,1] to [0,255] (W&B expects uint8)
            ...
            import cv2
            cv2.imwrite(f"logs/renders/step_{self.global_step}/{tag_prefix}_grid.png", image_grid_np[:, :, ::-1])  # BGR → RGB
            print(f"✅ Saved {tag_prefix}_grid.png to logs/renders/step_{self.global_step}/")

            # Return numpy array for W&B logging
            ...
            import traceback
            traceback.print_exc()
            # Return dummy white image to avoid crash
